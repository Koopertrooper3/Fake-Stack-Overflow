...
async function createUser(username,password,email,reputation){
  try {
...
const populate = async () => {
    ...
    let user1 = await createUser("kooper","veryeasypassword","christian.yu@stonybrook.edu",50)
    let user2 = await createUser("looper","somewhatharderpassword","reimu.hakurei@stonybrook.edu",50)
    let naughtyuser = await createUser("marisa.kirisame","somewhatharderpassword","marisa.kirisame@stonybrook.edu",30)

    let ansuser1 = await createUser("hong.meiling","scarletmansion1","hong meiling@stonybrook.edu",50)
    let ansuser2 = await createUser("sakuya.izayoi","scarletmansion2","sakuya.izayoi@stonybrook.edu",50)
    let ansuser3 = await createUser("patchouli.knowledge","scarletmansion3","patchouli.knowledge@stonybrook.edu",50)
    let ansuser4 = await createUser("lastansuser","decentpassword","lastansuser@gmail.com",50)
    ...
    await questionCreate('android studio save string shared preference, start activity and load the saved string', "sumamry2",'I am using bottom navigation view but am using custom navigation, so my fragments are not recreated every time i switch to a different view. I just hide/show my fragments depending on the icon selected. The problem i am facing is that whenever a config change happens (dark/light theme), my app crashes. I have 2 fragments in this activity and the below code is what i am using to refrain them from being recreated.', [t3, t4, t2], [a3, a4, a5], user2, false, 121);
    if(db) db.close();
    console.log('done');
  }
